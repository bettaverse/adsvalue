$(function () {
  ...
  let pickedEmoji = "👀";
  for (let b = 0; b < emojiBtns.length; b++) {
    var bt = $("<button />", {
      text: emojiBtns[b],
      value: emojiBtns[b],
      click: function (e) {
        e.preventDefault();
        pickedEmoji = $(this).val();
        // alert($(this).val());
      },
    })
      .wrap("<tr><td></td></tr>")
  ...
  //   test: "test",
  // });
  // chrome.storage.sync.get({ test: "test" }, function (val) {
  //   if (val.test === "test") {
  //     console.log("true");
  //     tasksList = ["one", "two", "three"];
  //   } else if (val.test === "not a test") {
  //     console.log("false");
  //   }
  //   console.log("val.test: " + val.test);
  // });

  chrome.storage.sync.get({ list1: [] }, function (val) {
    if (val.list1.length === 0) tasksList = initialArray;
    if (val.list1.length > 0) tasksList = val.list1;
    console.log("val.list1 :" + val.list1);
    //displaying the old items
    for (var i = 0; i < tasksList.length; i++) {
      addListItem(tasksList[i].ad, tasksList[i].emoji);
    }
  });

  // chrome.storage.sync.get(["list2"], function (val) {
  //   if (val.list2.length > 0) learnList = val.list2;
  ...
  $("#addButtonTask").click(function () {
    ...
    // var newEmoji = $("#emoji").val();
    //adding the new item to tasklist array
    tasksList.push({ ad: newTask, emoji: pickedEmoji });
    // tasksList.push(newEmoji);

    console.log("tasksList under click :" + tasksList);
    addListItem(newTask, pickedEmoji);
    // addListItem(newEmoji);
  ...
  function addListItem(value1, value2) {
...
//   //       "Task checklist for " + day + ", " + todayDate.getDate() + " " + month;
//   //   }
// });
//

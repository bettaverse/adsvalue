...
function displayName(channelData) {
  var name = channelData.name,
    param = channelData.param;
  name = name.replace(/Channel$/, "");
  name = name.replace(/([^A-Z])([A-Z])/g, "$1 $2"); // Spaces b/w words
  name = name.replace(/ Block/g, "Block"); // AdBlock, ads value...
...
function setStatus(disabled) {
  $("#flickr-param, #btnGo, .channel-ui > div > input").prop(
    "disabled",
    disabled
  );
}
...
// Disable Ads ValueOptions, when ad replacement is disabled

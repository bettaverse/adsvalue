...
if (!SAFARI) {
    ...
    track: function (details) {
      ...
      if (undefined === fd.get(tabId, potentialEmptyFrameId)) {
        ...
        log(
          ...
          "Null frame",
          tabId,
          potentialEmptyFrameId,
          "found; giving it the tab's URL."
        );
      }
  ...
  function onBeforeRequestHandler(details) {
    ...
    var sub_frame =
      details.frameId !== 0 ? frameData.get(tabId, details.frameId) : null;

    // If top frame is whitelisted, don't process anything
    if (top_frame.whitelisted) {
      log(
        "[DEBUG]",
        "Ignoring whitelisted tab",
        tabId,
    ...
    var requestingFrameId =
      reqType === "sub_frame" ? details.parentFrameId : details.frameId;
    ...
    var matchGeneric = _myfilters.blocking.whitelist.matches(
      top_frame.url,
    ...
    if (details.frameId !== 0) {
      if (!matchGeneric) {
        ...
        matchGeneric = _myfilters.blocking.whitelist.matches(
          sub_frame.url,
          ElementTypes.genericblock,
          sub_frame.url
        );
    ...
    var blocked = _myfilters.blocking.matches(
      details.url,
      elType,
      frameDomain,
      false,
    ...
    if (blocked && frameDomain === "www.hulu.com") {
      if (
        frameData.get(tabId, 0).domain !== "www.hulu.com" &&
        /ads\.hulu\.com/.test(details.url)
      ) {
        // good enough
        blocked = false;
      }
    ...
    var canPurge =
      elType &
      (ElementTypes.image | ElementTypes.subdocument | ElementTypes.object);
    if (canPurge && blocked) {
      ...
      var frameUrl = frameData
        .get(tabId, requestingFrameId)
        .url.replace(/#.*$/, "");
      var data = {
  ...
  function onCreatedNavigationTargetHandler(details) {
    ...
    var match = _myfilters.blocking.matches(
      url,
      ElementTypes.popup,
      opener.domain,
      false,
...
function confirm_removal_of_custom_filters_on_host(host, activeTab) {
  ...
  var confirmation_text = translate("confirm_undo_custom_filters", [
    custom_filter_count,
    host,
  ]);
  if (!FIREFOX) {
    // Firefox doesn't support confirm() in BG page yet
  ...
  if (SAFARI && !get_settings().safari_content_blocking) {
    if (newValue) {
      safari.extension.removeContentScript(
        safari.extension.baseURI + "js/adblock_safari_beforeload.js"
      );
    } else {
...
if (!SAFARI) {
    ...
    chrome.browserAction.getBadgeText({}, function (result) {
      if (result === "New!") {
        ...
        chrome.browserAction.setIcon(
          { tabId: options.tabId, path: options.iconPaths },
          iconCallback
        );
      }
    });
  ...
  function updateBadge(tabId) {
    ...
    var isBlockable =
      !page_is_unblockable(main_frame.url) &&
      !page_is_whitelisted(main_frame.url) &&
      !/chrome\/newtab/.test(main_frame.url);
  ...
  function updateButtonUIAndContextMenus() {
    ...
    function setBrowserButton(info) {
      ...
      if (adblock_is_paused()) {
        browsersBadgeOptions.iconPaths = {
          19: "img/icon19-grayscale.png",
          38: "img/icon38-grayscale.png",
        };
        setBrowserActions(browsersBadgeOptions);
      } else if (
        info.disabled_site &&
...
function injectjQueryUI() {
  if (SAFARI) {
    safari.extension.addContentScriptFromURL(
      safari.extension.baseURI + "lib/jquery-ui.min.js",
      [],
      [],
      false
    );
...
function get_content_script_data(options, sender) {
  ...
  var hiding =
    running && !page_is_whitelisted(sender.url, ElementTypes.elemhide);
  ...
  if (
    ...
    if (sender.tab.url === sender.url) {
      result.selectors = _myfilters.hiding.filtersFor(
        options.domain,
        matchGeneric
      );
    } else {
  ...
  var emit_page_broadcast = (function () {
    var injectMap = {
      ...
      send_content_to_back: {
        allFrames: true,
        include: ["uiscripts/send_content_to_back.js"],
      },
    };
...
function launch_subscribe_popup(loc) {
  window.open(
    chrome.runtime.getURL("pages/subscribe.html?" + loc),
    "_blank",
...
function process_frameData(fd) {
  for (var frameId in fd) {
    ...
    for (var resource in frameResources) {
      ...
      res.blockedData = _myfilters.blocking.matches(
        res.url,
        res.elType,
        res.frameDomain,
        true,
  ...
  if (
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      chrome.notifications.onButtonClicked.addListener(function (
        ...
        if (buttonIndex === 0) {
          openTab(
            "http://help.getadblock.com/support/solutions/articles/6000055822-i-m-seeing-similar-ads-on-every-website-"
          );
        }
        if (buttonIndex === 1) {
...
if (!SAFARI) {
  ...
  if (chrome.webNavigation && chrome.webNavigation.onCreatedNavigationTarget) {
    chrome.webNavigation.onCreatedNavigationTarget.addListener(
      onCreatedNavigationTargetHandler
    );
  }
...
function getDebugInfo(callback) {
  // Is this installed build of Ads Valuethe official one?
  function getBuildInfo(callback) {
    chrome.management.getSelf(function (selfData) {
      ...
      if (
        extensionID !== "mdcgnhlfpnbeieiiccmebgkfdebafodo" &&
        extensionID !== "pejeadkbfbppoaoinpmkeonebmngpnkk" &&
        extensionID !== "catblock@ads value.tk"
      ) {
  ...
  // Push Ads Valueversion and build to |the_debug_info| object
  function processDebugInfo(buildInfo) {
    ...
    for (var id in get_subscriptions) {
      if (get_subscriptions[id].subscribed) {
        ...
        the_debug_info.filter_lists.push(
          "  last updated: " +
            new Date(get_subscriptions[id].last_update).toUTCString()
        );
      }
    }
    ...
    for (setting in settings) {
      the_debug_info.settings.push(
        setting + ": " + JSON.stringify(settings[setting]) + "\n"
      );
    }
...
function makeReport() {
  ...
  body.push("");
  body.push(
    "If unchecking all filter lists fixes the problem, which one filter" +
      "list must you check to cause the problem again after another restart?"
  );
  body.push("");
  body.push(
    "Technical Chrome users: Go to chrome://extensions ->" +
...
// Listens for message from Ads Valuecontent script asking to load jQuery.
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.command === "inject_jquery") {
    if (!SAFARI) {
      chrome.tabs.executeScript(
        undefined,
        { allFrames: request.allFrames, file: "../lib/jquery.min.js" },
        function () {
          sendResponse({});
        }
      );
      ...
      safari.extension.addContentScriptFromURL(
        safari.extension.baseURI + "lib/jquery.min.js",
        [],
        [],
        false
      );

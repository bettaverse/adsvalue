...
safari.application.addEventListener(
  ...
  function (messageEvent) {
    if (
      ...
      if (
        !messageEvent.target.url ||
        messageEvent.target.url === args[1].tab.url
      ) {
        frameData.create(
          messageEvent.target.id,
          args[1].tab.url,
          args[0].domain
        );
      } else if (
        messageEvent.target.url === frameData.get(messageEvent.target.id).url
      ) {
    ...
    if (
      adblock_is_paused() ||
      page_is_unblockable(sendingTab.url) ||
    ...
    if (!isPopup) {
      ...
      var frameDomain = parseURI.getUnicodeDomain(
        messageEvent.message.frameDomain
      );
      var isMatched =
        url && _myfilters.blocking.matches(url, elType, frameDomain);
      if (isMatched) {
      ...
      if (messageEvent.message.referrer) {
        var isMatched = _myfilters.blocking.matches(
          sendingTab.url,
          ElementTypes.popup,
...
var ABPopover = safari.extension.createPopover(
  "CatBlock",
  safari.extension.baseURI + "button/popup.html"
);
  ...
  function (event) {
    if (event.target instanceof SafariExtensionToolbarItem) {
      ...
      if (item.browserWindow && !item.popover) {
        ...
        // lacking a popover (which only happens on browser startup or when the user removes Ads Valuetoolbar item and later
        // drags it back).
...
function set_content_scripts() {
  if (get_settings().safari_content_blocking) {
    safari.extension.addContentScriptFromURL(
      safari.extension.baseURI + "js/adblock_safari_contentblocking.js",
      [],
      [],
      false
    );
...
safari.application.addEventListener(
  "beforeNavigate",
  function (event) {
    // Remove bandaids.js from YouTube.com when a user pauses Ads Valueor if the enabled click to flash compatibility mode
    if (
      /youtube.com/.test(event.url) &&
      (is_adblock_paused() ||
        get_settings().clicktoflash_compatibility_mode === true)
    ) {
      safari.extension.removeContentScript(
        safari.extension.baseURI + "js/bandaids.js"
      );
    }
    ...
    if (
      ...
      !parseURI.parseSearch(event.url).ab_channel
    ) {
      safari.extension.addContentScriptFromURL(
        safari.extension.baseURI + "js/ytchannel.js",
        [],
        [],
        false
      );
...
safari.application.addEventListener(
  ...
  function (event) {
    ...
    if (command === "CatBlockOptions") {
      ...
      command in
      {
        "show-whitelist-wizard": 1,
        "show-blacklist-wizard": 1,
        "show-clickwatcher-ui": 1,
      }
    ) {
...
safari.application.addEventListener(
  ...
  function (event) {
    ...
    event.contextMenu.appendContextMenuItem(
      "show-blacklist-wizard",
      translate("block_this_ad")
    );
    event.contextMenu.appendContextMenuItem(
      "show-clickwatcher-ui",
      translate("block_an_ad_on_this_page")
    );

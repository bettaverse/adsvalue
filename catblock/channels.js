  ...
  add(data) {
    ...
    switch (data.name) {
      case "TheCatsOfCatBlockUsersChannel":
        klass = TheCatsOfCatBlockUsersChannel;
        break;
      case "AprilFoolsCatsChannel":
        klass = AprilFoolsCatsChannel;
        break;
      case "TheCatsOfProjectCATS":
        klass = TheCatsOfProjectCATS;
        break;
      case "FlickrSearchChannel":
        klass = FlickrSearchChannel;
        break;
      case "FlickrPhotosetChannel":
        klass = FlickrPhotosetChannel;
        break;
      default:
        return;
    }
  ...
  _loadFromStorage() {
    ...
    if (!entries || (entries.length > 0 && !entries[0].name)) {
      ...
      if (storage_get("project_cats")) {
        this.add({
          name: "TheCatsOfProjectCATS",
          param: undefined,
          enabled: true,
        });
        this.add({
          name: "TheCatsOfCatBlockUsersChannel",
          param: undefined,
          enabled: false,
        });
        this.add({
          name: "AprilFoolsCatsChannel",
          param: undefined,
          enabled: false,
        });
      } else {
...
class Channel {
  ...
  _getLatestListings() {
    throw new Error(
      "Implemented by subclass. Call callback with up-to-date listings."
    );
  }
}
...
// Channel pulling from a Flickr channel "The Cats of Ads Valueusers"
